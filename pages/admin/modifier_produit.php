...
if (isset($_GET['id'])) {
    ...
    try {
        ...
        $message = "<div class='message error'>❌ Erreur de récupération du produit : " . $e->getMessage() . "</div>";
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["modifier_produit"])) {
    ...
    $imageName = $produit['image']; // Garder l'ancienne image par défaut
    ...
    try {
        $stmt = $conn->prepare("UPDATE produit SET nom = ?, description = ?, prix = ?, stock = ?, image = ?, id_categorie = ? WHERE id_produit = ?");
        if ($stmt->execute([$nom, $description, $prix, $stock, $imageName, $categorie, $idProduit])) {
            $message = "<div class='message success'>✅ Produit mis à jour avec succès !</div>";
        } else {
            $message = "<div class='message error'>❌ Erreur lors de la mise à jour du produit.</div>";
        }
    } catch (PDOException $e) {
        $message = "<div class='message error'>❌ Erreur de mise à jour : " . $e->getMessage() . "</div>";
    }
}
    ...
    <main class="main-content">
        <div class="form-wrapper">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <label>Prix :</label>
                <input type="number" name="prix" min="0" step="0.01" value="<?= htmlspecialchars($produit['prix']); ?>" required>

                <label>Stock :</label>
                <input type="number" name="stock" min="0" value="<?= htmlspecialchars($produit['stock']); ?>" required>

                <label>Image :</label>
                <div class="drop-zone" id="dropZone">
                    <img src="../../images/produits/<?= htmlspecialchars($produit['image']); ?>" alt="Image actuelle" width="100">
                ...
                <button type="submit" name="modifier_produit">Mettre à jour le produit</button>
            </form>
        </div>
    </main>
